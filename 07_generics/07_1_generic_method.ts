...
let newObj = addUID({ name: 'yoshi', age: 23 });
...
let addUID_2 = <T>(obj: T) => {
    let uid = Math.floor(Math.random() * 1000);
    return { ...obj, uid };
}

let newObj_2 = addUID_2({ name: 'Hamamoto', age: 47 });     // Captures all the props of the object while invoking the func here
...
let addUID_3 = <T extends object>(obj: T) => {
    let uid = Math.floor(Math.random() * 1000);
    return { ...obj, uid };
}
...
xyz = [1, 2, 3, 4, 5, 6, 7];
let newObj_3 = addUID_3(xyz);   // Though I can pass an array of numbers while the func is expecting an object, it's the defficiency of typescript.
...
let addUID_4 = <T extends { name: string }>(obj: T) => {
    let uid = Math.floor(Math.random() * 1000);
    return { ...obj, uid };
}

// let newObj_4 = addUID_4({ age: 43 });   // Throw error, since no "name" prop is defined while passing the obj
let newObj_4 = addUID_4({ name: `Ashimi`, age: 24 });     // At least, the name prop required to be defined while passing an obj
